...
/**
 * @note I understand this is not as easy to read than a regular <template>
 * block, but a render function is necessary to avoid compiler output
 * differences between vue2 and vue3 breaking code compatibility.
 *
 * IMPORTANT NOTES:
 ...
 *  - If refactoring to composition-api and thus dropping supporr to vue<@2.6
 ...
 *
 * If you do a change make sure the test snapshots changes are expected.
 *
            ...
            h(
              'div',
              {
                ...
                ...(isVue3
                  ...
                  : { on: { scroll: (event) => emit('scroll', event) } }
                )
              },
              [
                h(
...
export default {
  ...
  render() {
    return renderFn({ emit: this.$emit, slots: isVue3 ? this.$slots : this.$scopedSlots })
  }
}
